    ...
    <div class="fixed inset-0 bg-gray-900 bg-opacity-30 z-50 flex items-center justify-center" x-show="modalOpen" x-cloak>
        <div class="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-lg max-w-lg w-full"
            ...
            <div class="flex justify-center items-center">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Edit Permintaan Barang</h3>
            </div>

            <!-- Form Edit -->
            <form
                action="{{ route('work_request.work_rabs.update', ['id' => $workRequest->id, 'work_rab_id' => $rabRequest ? $rabRequest->id : '']) }}"
                method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-2 gap-4">
                        ...
                        <select id="work_request_item_id" name="work_request_item_id"
                            ...
                            @foreach ($itemRequest as $item)
                                <option value="{{ $item->id }}"
                                    {{ $rabRequest && optional($rabRequest->workRequestItem)->id == $item->id ? 'selected' : '' }}>
                                    {{ $item->item_name }}
                                </option>
                            @endforeach
                        </select>
                        <x-input-error for="work_request_item_id" class="mt-2" />
                    </div>
                    ...
                    <div>
                        ...
                        <x-input id="harga" class="mt-1 block w-full min-h-[40px]" type="text" name="harga"
                            value="{{ old('harga', $rabRequest ? number_format($rabRequest->harga, 0, ',', '.') : '') }}"
                ...
                <div class="flex justify-end gap-2 mt-4">
                    <x-button.button-action color="red" class="px-4 py-2 bg-gray-500 text-white rounded-md"
                        @click="modalOpen = false" type="button">Batal</x-button.button-action>
                    <x-button.button-action color="teal" type="submit">Simpan</x-button.button-action>
                </div>
            </form>
        </div>
    </div>
